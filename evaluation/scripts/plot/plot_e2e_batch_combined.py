...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Input arch name (overrides other options)",
    )
...
models_to_plot = ["BitNet 3B", "Llama3 8B", "Falcon 1B"]
...
def read_batch_csv_files(directory, arch):
    ...
    for csv_file in csv_files:
        try:
            ...
            if (
                model_quant in E2E_TYPE_VARIANTS
                and model_quant not in E2E_TYPE_DEVICE_MAP[arch]
            ):
                continue
...
def plot_all_archs_e2e_batch(
    results_dict, model_names=None, thread_mode="auto", tg_value=None
):
    """
    ...
    if tg_value is not None:
        for arch in results_dict:
            results_dict[arch] = results_dict[arch][
                results_dict[arch]["TG"] == tg_value
            ]
    else:
        ...
        for row_idx, model in enumerate(model_names):
            ...
            if not subset.empty:
                ...
                for b_idx, batch_size in enumerate(all_batch_sizes):
                    ...
                    available_quants = sorted(
                        b_data["model_quant"].unique(),
                    ...
                    bar_width = (
                        total_width / num_quants if num_quants > 0 else total_width
                    )
                    ...
                    for q_idx, quant in enumerate(available_quants):
                        ...
                        if not quant_data.empty:
                            ...
                            if num_quants % 2 == 0:  # Even number of bars
                                start = (
                                    x_positions[b_idx]
                                    - (bar_width * num_quants) / 2
                                    + bar_width / 2
                                )
                            else:  # Odd number of bars
                                start = (
                                    x_positions[b_idx]
                                    - (bar_width * (num_quants - 1)) / 2
                                )

                            pos = start + q_idx * bar_width

                            # Get style from E2E_TYPE_STYLES mapping
                            style = E2E_TYPE_STYLES.get(
                                quant, {"color": "#000000", "hatch": ""}
                            )
            ...
            if col_idx == 0 and row_idx == 1:
                ax.set_ylabel(
                    "Throughput (tokens/s)", fontsize=24, fontweight="bold", labelpad=10
                )
    ...
    for quant in all_quants:
        ...
        dummy_bar = legend_ax.bar(
            0, 0, color=style["color"], hatch=style["hatch"], edgecolor="black"
        )
...
def generate_speedup_reports(
    results_dict, models_to_plot, output_dir, lut2_on=None, entry_size=None
):
    ...
    for arch, df in results_dict.items():
        ...
        if "E2E_TYPE_DEVICE_MAP" in globals() and arch in E2E_TYPE_DEVICE_MAP:
            comparison_types = (
                set([E2E_TYPE_MAP[t] for t in E2E_TYPE_DEVICE_MAP[arch]])
                & relevant_types
            )
        else:
        ...
        for model in models_to_plot:
            ...
            subset = subset.drop_duplicates(
                subset=["model_name", "model_quant"], keep="first"
            )
            ...
            if not subset.empty:
                ...
                for comp_type in comparison_types:
                    ...
                    for baseline_type in relevant_types:
                        if (
                            baseline_type not in perf_by_type
                            or baseline_type == comp_type
                        ):
                            continue
        ...
        if all_speedups:
            ...
            min_speedups = (
                ...
                .min()
                .reset_index()
                .rename(columns={"speedup": "min_speedup"})
            )
            max_speedups = (
                grouped["speedup"]
                .max()
                .reset_index()
                .rename(columns={"speedup": "max_speedup"})
            )

            # Get model combinations for min and max speedups
            min_model = grouped.apply(
                lambda x: x.loc[x["speedup"].idxmin(), "model"], include_groups=False
            ).reset_index(name="min_speedup_model")
            max_model = grouped.apply(
                lambda x: x.loc[x["speedup"].idxmax(), "model"], include_groups=False
            ).reset_index(name="max_speedup_model")

            # Merge all stats
            stats_df = avg_speedups.merge(
                min_speedups, on=["comparison_type", "baseline_type"]
            )
            stats_df = stats_df.merge(
                max_speedups, on=["comparison_type", "baseline_type"]
            )
            stats_df = stats_df.merge(
                min_model, on=["comparison_type", "baseline_type"]
            )
            stats_df = stats_df.merge(
                max_model, on=["comparison_type", "baseline_type"]
            )
...
def main():
    ...
    if args.both:
        ...
        results_dict_single = load_results_for_all_archs(
            all_archs, single_thread_config
        )
        ...
        if results_dict_single:
            ...
            for tg in sorted(all_tg_values):
                ...
                generate_speedup_reports(
                    results_dict_single, models_to_plot, reports_dir_single
                )
                ...
                if fig_single:
                    ...
                    output_file_single = os.path.join(
                        output_dir, f"e2e_batch_comparison_single_thread_TG{tg}.pdf"
                    )
                    fig_single.savefig(output_file_single, dpi=300, bbox_inches="tight")
                    print(
                        f"Single-thread comparison plot for TG={tg} saved to {output_file_single}"
                    )
        ...
        if results_dict_multi:
            ...
            for tg in sorted(all_tg_values):
                ...
                if fig_multi:
                    ...
                    output_file_multi = os.path.join(
                        output_dir, f"e2e_batch_comparison_multi_thread_TG{tg}.pdf"
                    )
                    fig_multi.savefig(output_file_multi, dpi=300, bbox_inches="tight")
                    print(
                        f"Multi-thread comparison plot for TG={tg} saved to {output_file_multi}"
                    )
    else:
            ...
            for tg in sorted(all_tg_values):
                ...
                if fig:
                    ...
                    output_file = os.path.join(
                        output_dir, f"e2e_batch_comparison_{title_suffix}_TG{tg}.pdf"
                    )
                    fig.savefig(output_file, dpi=300, bbox_inches="tight")
                    print(
                        f"End-to-end batch comparison plot for TG={tg} saved to {output_file}"
                    )
