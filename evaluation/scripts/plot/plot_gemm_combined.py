...
def parse_gemm_results(filepath):
    ...
    try:
        ...
        df = df.rename(
            columns={"name": "type_a", "uspr": "us_per_run", "rps": "runs_per_sec"}
        )
...
def extract_file_metadata(filename):
    ...
    # Older result files encode LUT2 state explicitly as _lon_ / _loff_.
    # Newer bench-gemm.sh outputs omit it, so treat the field as optional.
    lut2_pattern = re.search(r"_(lon|loff)_", basename_no_ext)
    lut2_on = lut2_pattern.group(1) == "lon" if lut2_pattern else False
...
def plot_multi_arch_comparison(
    results_dict, mkn_to_plot=None, lut2_on=None, entry_size=None, thread_mode="auto"
):
        ...
        for arch_idx, arch in enumerate(archs):
            ...
            for col_offset in range(n_cols_per_arch):
                ...
                subset = subset.drop_duplicates(
                    subset=["m", "n", "k", "type_a"], keep="first"
                )
                ...
                if not subset.empty:
                    ...
                    subset = subset.sort_values(
                        "type_a", key=lambda x: x.map(TYPE_ORDER).fillna(999)
                    )
                    ...
                    for i, type_a in enumerate(type_a_values):
                        style = GEMM_TYPE_STYLES.get(
                            type_a, {"color": "#000000", "hatch": ""}
                        )
                        bar = ax.bar(
                            x_pos[i],
                            performance[i],
                ...
                if arch_idx == 0 and row == 1 and col_offset == 0:
                    ax.set_ylabel(
                        "Speed (runs/sec)", fontsize=24, fontweight="bold", labelpad=10
                    )
...
def generate_speedup_reports(
    results_dict, mkn_to_plot, output_dir, lut2_on=None, entry_size=None
):
    ...
    for arch, df in results_dict.items():
        ...
        if "GEMM_TYPE_DEVICE_MAP" in globals() and arch in GEMM_TYPE_DEVICE_MAP:
            comparison_types = (
                set([GEMM_TYPE_MAP[t] for t in GEMM_TYPE_DEVICE_MAP[arch]])
                & relevant_types
            )
        else:
        ...
        if all_speedups:
            ...
            min_mnk = grouped.apply(
                lambda x: x.loc[x["speedup"].idxmin(), "mnk"], include_groups=False
            ).reset_index(name="min_speedup_mnk")
            max_mnk = grouped.apply(
                lambda x: x.loc[x["speedup"].idxmax(), "mnk"], include_groups=False
            ).reset_index(name="max_speedup_mnk")
...
def main():
    ...
    if args.both:
        ...
        results_dict_single = load_results_for_all_archs(
            all_archs, single_threads_config
        )

        fig_single = plot_multi_arch_comparison(
            results_dict_single, mkn_to_plot=combinations_to_plot, thread_mode="single"
        )
        ...
        generate_speedup_reports(
            results_dict_single, combinations_to_plot, reports_dir_single
        )
        ...
        fig_multi = plot_multi_arch_comparison(
            results_dict_multi, mkn_to_plot=combinations_to_plot, thread_mode="multi"
        )
        # Generate speedup reports for multi-thread configuration
        reports_dir_multi = eval_path("reports_gemm", "multi_thread")
        generate_speedup_reports(
            results_dict_multi, combinations_to_plot, reports_dir_multi
        )
        ...
        fig = plot_multi_arch_comparison(
            results_dict, mkn_to_plot=combinations_to_plot, thread_mode=thread_mode
        )

        # Generate speedup reports
        reports_dir = eval_path("reports", f"{title_suffix}")
        generate_speedup_reports(results_dict, combinations_to_plot, reports_dir)
