    ...
    if args.both:
        ...
        if results_dict_single:
            fig_single = plot_all_archs_e2e_prefill(
                results_dict_single, model_names=models_to_plot, thread_mode="single"
            )
            ...
            os.makedirs(eval_path("figures"), exist_ok=True)
            output_file_single = eval_path(
                "figures", "e2e_prefill_comparison_single_thread.pdf"
            )
        ...
        if results_dict_multi:
            fig_multi = plot_all_archs_e2e_prefill(
                results_dict_multi, model_names=models_to_plot, thread_mode="multi"
            )

            # Generate speedup reports for multi-thread configuration
            reports_dir_multi = eval_path("reports_e2e_prefill", "multi_thread")
            generate_speedup_reports(
                results_dict_multi, models_to_plot, reports_dir_multi
            )

            # Save the multi-thread plot
            os.makedirs(eval_path("figures"), exist_ok=True)
            output_file_multi = eval_path(
                "figures", "e2e_prefill_comparison_multi_thread.pdf"
            )
        ...
        if results_dict:
            ...
            fig = plot_all_archs_e2e_prefill(
                results_dict, model_names=models_to_plot, thread_mode=thread_mode
            )

            os.makedirs(eval_path("figures"), exist_ok=True)
            output_file = eval_path(
                "figures", f"e2e_prefill_comparison_{title_suffix}.pdf"
            )
